...
public class ClassPathOpener {
     ...
     */
    public ClassPathOpener(byte[] jarArray, boolean sort, Consumer consumer) {
    ...
    public boolean process() {
        ...
        return processOne(jarArray, true);
    }
    ...
    //edit by Tim Wilking [remove] (File)
    private boolean processOne(byte[] jarArray, boolean topLevel) {
        try {
          /*  if (file.isDirectory()) {
            ...
            if (path.endsWith(".zip") ||
                    path.endsWith(".jar") ||
                    path.endsWith(".apk")) {*/
                return processArchive(jarArray);
          //  }
    ...
    //edit by Tim Wilking [remove] (complete Method)
   /* private boolean processDirectory(File dir, boolean topLevel) {
     ...
     */
    private boolean processArchive(byte[] jarArray) throws IOException {
       
    	ByteArrayInputStream byteInput = new ByteArrayInputStream(jarArray);
    	ZipInputStream zipInput = new ZipInputStream(byteInput); 
    	   	
    	ArrayList<java.util.zip.ZipEntry> entriesList 
        ...
        consumer.onProcessArchiveStart();

        byteInput = new ByteArrayInputStream(jarArray);
        zipInput = new ZipInputStream(byteInput);
        ...
        for (ZipEntry one: entriesList) {
            ...
            do{
               ...
               bytesRead = zipInput.read(buf, 0, buf.length); 
