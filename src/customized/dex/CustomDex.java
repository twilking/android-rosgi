...
public class CustomDex {
    ...
    public static byte[] dex( byte[] jarArray) {
    ...
    //args.outName = "ProxDex.jar";
    processOne(jarArray);
    
    byte[] outArray = writeDex();
   
    return createJar(outArray);
    ...
    //edit by Tim Wilking [remove] (Pathname, File)
    private static boolean processOne(byte[] jarArray) {
        ClassPathOpener opener;

        opener = new ClassPathOpener(jarArray, false,
                new ClassPathOpener.Consumer() {
            public boolean processFileBytes(String name, byte[] bytes) {
                return CustomDex.processFileBytes(name, bytes);
            }
        	...
        	public void onProcessArchiveStart() {
                if (args.verbose) {
                   DxConsole.out.println("processing archive "  + "...");
                }
            }
        });
     ...
     */
    private static byte[] createJar(byte[] dexArray) {
        /*
     ...
     /* FileOutputStream fw;
		try {
			fw = new FileOutputStream("C:" + sep + "Test.jar");
